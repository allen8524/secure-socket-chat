...
실험적 E2E whisper outer packet 예시입니다. `text` 평문은 outer packet에 포함하지 않고, `ciphertext` 안의 inner payload에만 들어갑니다.
...
```json
{
  ...
  "to": "bob",
  "from": "alice",
  "sender_e2e_public_key": "base64-public-key",
  "sender_e2e_fingerprint": "AA:BB:CC:DD:EE:FF:00:11",
  "recipient_e2e_fingerprint": "11:22:33:44:55:66:77:88",
  "ciphertext": "base64-inner-ciphertext",
  "algorithm": "PyNaCl Box",
  "e2e_version": 1,
  "sequence": 15,
  "payload_size": 0
}
...
암호화 전 inner payload 예시입니다. 이 JSON은 송신 클라이언트와 수신 클라이언트 사이의 E2E `Box`로 암호화되며, 서버는 복호화하지 않습니다.
...
| `e2e_whisper` | client -> server -> client | 실험적 1:1 E2E whisper outer packet |
| `image` | both | 이미지 전송 |
| `file` | both | 일반 파일 전송 |
...
- `e2e_whisper`는 outer packet에 평문 `text`를 넣지 않고, 수신 클라이언트가 자신의 E2E private key와 송신자 E2E public key로 inner payload를 복호화
